...
// Bridge Token: exchange a Supabase access_token for a custom app session JWT.
// Called after supabase.auth.signInWithPassword() succeeds to connect Supabase Auth
// with the custom OAuth JWT system used by tRPC/protectedProcedure.
export async function bridgeToken(supabaseAccessToken: string): Promise<{ sessionToken: string; user: any }> {
  const baseUrl = getApiBaseUrl();
  const url = `${baseUrl}/api/auth/bridge`;
  console.log("[API] bridgeToken: exchanging Supabase token for app JWT...");
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error("[API] bridgeToken failed:", response.status, errorText);
      throw new Error(`Bridge token failed: ${response.status} ${errorText}`);
    }

    const result = await response.json();
    console.log("[API] bridgeToken success: got app JWT");
    return {
