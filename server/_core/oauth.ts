...
export function registerOAuthRoutes(app: Express) {
  ...
  app.get("/api/auth/me", async (req: Request, res: Response) => {
    try {
      ...
      res.json({ user: buildUserResponse(result.user) });
    } catch (error) {
      // authenticateRequest only throws when JWT is invalid → true auth failure
      console.error("[Auth] /api/auth/me failed:", error);
      res.status(401).json({ error: "Not authenticated", user: null });
    }
  });

  // Bridge endpoint: exchange a Supabase access_token for a custom app session JWT.
  // Called by the mobile client after supabase.auth.signInWithPassword() succeeds.
  // This connects the Supabase Auth world to the custom OAuth JWT world.
  app.post("/api/auth/bridge", async (req: Request, res: Response) => {
    try {
      ...
      // Use Supabase user ID as openId (unique identifier)
      ...
      console.error("[Auth] /api/auth/bridge failed:", error);
      res.status(500).json({ error: "Bridge token exchange failed" });
    }
  });
  ...
  app.post("/api/auth/session", async (req: Request, res: Response) => {
    try {
      ...
      res.json({ success: true, user: buildUserResponse(result.user) });
    } catch (error) {
      // authenticateRequest only throws when JWT is invalid → true auth failure
